...
func execTest(t *testing.T, parameters *internal.ModulesDTO, in, out string) {
	...
	operation.KymaTemplate = internal.GetFileWithTest(t, fmt.Sprintf("%s/%s", kymaAppendModulesTestAssets, in))
	expectedKymaTemplate := internal.GetFileWithTest(t, fmt.Sprintf("%s/%s", kymaAppendModulesTestAssets, out))
...
func TestKymaAppendModulesWithEmptyDefaultOnes4_Run(t *testing.T) {
	...
	execTest(t, params(), givenKymaTemplateWithoutModules, givenKymaTemplateWithoutModules)
}
