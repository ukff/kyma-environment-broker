...
func (r readSession) GetOperationStatsV2() ([]dbmodel.OperationStatEntryV2, error) {
	...
	_, err := r.session.SelectBySql(fmt.Sprintf("select count(*), type, state, provisioning_parameters ->> 'plan_id' AS plan_id from %s group by type, state, provisioning_parameters ->> 'plan_id' where state='%s'", OperationTableName, domain.InProgress)).Load(&rows)
	return rows, err
}
